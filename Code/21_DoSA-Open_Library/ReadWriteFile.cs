...
namespace gtLibrary
{
    ...
    public class CReadFile
    {
        ...
        //Marerial 데이터를 읽어온다
        public bool readMaterialMagnetData(string strFileFullName, string strMeterialName, ref double dMu, ref double dHc)
        {
            ...
            try
            {
                ...
                foreach (string strMatName in listMaterialNames)
                {
                    if (strMatName == strMeterialName)
                        bCheckName = true;
                }

                if (bCheckName == false)
                {
                    CNotice.printTrace("존재하지 않은 영구저석의 특성값을 얻으려고 하고 있다.");
                    return false;
                }
        ...
        //Marerial 데이터를 읽어온다
        public bool readMaterialBHData(string strFileFullName, string strMeterialName, ref List<double> listH, ref List<double> listB)
        {
            List<string> listString = new List<string>();
